app.filter('ftOrderfilt',function(){
      return function(str){
        console.info(str);
        if(!str){
         return 'NONE'; 
...
//json数据前端分页
app.filter('conowFootablePaging', function() {
...
app.filter('conowFootableSize', function() {
 ...
 * 响应式列表指令
 */
app.directive('conowFootable', ['$http', '$compile', '$timeout', '$parse',  
  function($http,$compile,$timeout, $parse) {
      return {
          restrict: 'E',
          template:function(e,iAttrs) {
            ...
            * 列表项为空时显示内容
            */
            iAttrs.noDataTip = iAttrs.noDataTip ? iAttrs.noDataTip : '暂无信息';
            ...
            var temp = '<div>                                                                                                                                                                                                                         ' +
              ...
              '     <span class="col col-xs v-middle">' + ft_search_name + '</span>' + 
              '     <div class="col"><conow-lazy-input ng-model="' + iAttrs.ftSearch + '.TITLE"></conow-lazy-input></div>' +
              '   </div>' +
  ...
  /*            var ftSearch = scope[iAttrs.ftSearch];*/
              ...
              */
              ...
              scope[orderByfun + "_asc"] = function(obj, field) {
                scope[obj]['ORDER_' + field] = field + ' asc ';
              };
              scope[orderByfun + "_desc"] = function(obj, field) {
                scope[obj]['ORDER_' + field] = field + ' desc ';
              };
              scope[orderByfun + "_none"] = function(obj, field) {
                scope[obj]['ORDER_' + field] = '';
              };
              
              var sort_templ = '';
              if (iAttrs.sortIndex && iAttrs.sortIndex.indexOf("0;")) {
              ...
              if(titles[i].indexOf('phone')>=0){
                ...
                if(items[i].indexOf(':btn')>=0){
                  ...
                  thead += '<th class="hidden-xs">' + 
                  sort_templ + titles[i].split(':')[0] + '</th>';
                ...
                thead+='<th>'+ sort_templ+ titles[i].split(':')[0]+'</th>';
                if(items[i].indexOf(':btn')>=0){                  
