...
angular.module('angular-send-feedback').directive('angularFeedback', [ '$location', function($location) {
    ...
    return {
        ...
        link: function($scope, $element, $attrs) {
        	// 登录页不显示反馈按钮
        	if($location.url().indexOf('access') > -1) return;
                ...
                $.feedback = function(options) {
                    ...
                    var tplCN = {
                        description: '<div id="feedback-welcome">' +
                            '<div class="feedback-logo">用户反馈</div>' +
                            '<p>亲爱的用户，感谢您使用我们的ARP3.0系统。欢迎你随时向我们反映系统中的问题，功能上的改进或者其他意见及建议。</p>' +
                            '<p>请在下面输入框中输入您的意见或建议:</p>' +
                            '<textarea id="feedback-note-tmp"></textarea>' +
                            '<p>请点击“下一步”按钮，开启“网页截图”功能，并标注出与您描述信息相关的页面区域，以帮助开发人员更加清晰的了解您反馈的问题。</p>' +
                            '<button id="feedback-welcome-next" class="feedback-next-btn feedback-btn-gray">下一步</button>' +
                            '<div id="feedback-welcome-error">请输入描述信息。</div>' +
                            '<div class="feedback-wizard-close"></div>' +
                        '</div>',

                        highlighter: '<div id="feedback-highlighter">' +
                        	'<div class="bg-primary feedback-highlighter-title">工具栏</div>'+
                            /*'<div class="feedback-logo" style="width: auto;"><span>用户反馈</span><span class="text-base text-dark">（若影响区域选择，可移动本对话框）</span></div>' +*/
                            /*'<div class="clearfix"><label class="i-checks pull-left"><input type="radio" checked onclick="javascript:highlight=1;" value="highlight" name="screenshotType"><i></i></label>' +*/
                            '<div class="clearfix">'+
                        		'<button class="feedback-sethighlight feedback-active"><i class="choice-sethighlight ci-choice choice-visible"></i><span>框选标记</span></button>' +
                            	/*'<label>框选页面上要突出显示的区域</label><span>(可多次操作)</span>'+*/
                            '</div>' +
                            /*'<div class="clearfix"><label class="i-checks"><input type="radio" onclick="javascript:highlight=0;" value="blackout" name="screenshotType"><i></i></label>' +*/
                            '<div class="clearfix">'+
	                            '<button class="feedback-setblackout"><i class="choice-setblackout ci-choice choice-hidden"></i><span>遮盖隐私</span></button>' +
	                            /*'<label class="lower">框选页面上要隐藏的区域</label><span class="lower">(可多次操作)</span>'+*/
                            '</div>' +
                            /*'<div class="feedback-buttons">' +
                                '<button id="feedback-highlighter-next" class="btn btn-primary">下一步</button>' +
                                '<button id="feedback-highlighter-back" class="feedback-back-btn feedback-btn-gray">上一步</button>' +
                            '</div>' +*/
                            /*'<div class="feedback-wizard-close"></div>' +*/
                        '</div>',
                        ...
                        overview: '<div id="feedback-overview">' +
                        	'<div style="position: relative; height: 100%;">'+
	                        	'<div class="feedback-overview-title">'+
	                        		'<div class="text-md" style="width: auto;"><i class="ci-feedback v-middle text-xlg m-r-xs"></i><span class="v-middle">用户反馈</span></div>' +
	                        	'</div>'+
	                        	'<div class="feedback-overview-content">'+
			                        ...
			                        '<div id="feedback-overview-description">' +
		                                '<div id="feedback-overview-description-text">' +
		                                    '<h3>意见/建议描述</h3>' +
			                                '<textarea id="feedback-overview-note"></textarea>'+
		                                '</div>' +
		                            '</div>' +
		                            '<span class="tips">请使用左侧工具在页面上标注,来帮助我们更好地理解您的反馈信息。</span>' +
		                        '</div>'+
	                            '<div class="feedback-buttons">' +
	                                '<button id="feedback-submit" class="btn btn-primary pull-right">提交</button>' +
	                                /*'<button id="feedback-overview-back" class="btn btn-default pull-right m-r-sm">上一步</button>' +*/
	                                '<div id="feedback-overview-error">请输入您的意见或建议</div>' +
	                            '</div>' +
	                            '<div class="feedback-wizard-close ci-close text-white text-lg"></div>' +
                            '</div>'+
                        '</div> ',

                        submitSuccess: '<div id="feedback-submit-success">' +
                            '<div class="feedback-logo" style="width: auto;">用户反馈</div>' +
                            '<p>感谢您的反馈信息！</p>' +
                            '<p>正是因为您珍贵的意见/建议，我们才可以将ARP3.0更加完善！</p>' +
                            '<button class="btn btn-primary pull-right feedback-close-btn">确定</button>' +
                            '<div class="feedback-wizard-close"></div>' +
                        '</div>',

                        submitError: '<div id="feedback-submit-error">' +
                            '<div class="feedback-logo" style="width: auto;">用户反馈</div>' +
                            '<p>哦哦~，因为某些不知名错误导致您的反馈尚未记录，请稍后重新提交。</p>' +
                            '<button class="btn btn-primary pull-right feedback-close-btn">确定</button>' +
                            '<div class="feedback-wizard-close"></div>' +
                        '</div>' 
                    };
                    ...
                    var settings = $.extend({
                            ...
                            initButtonText:         '点我吐槽',//'Send feedback',
                            strokeStyle:            'black',
                            shadowColor:            'black',
                            shadowOffsetX:          1,
                            shadowOffsetY:          1,
                        ...
                        if (supportedBrowser) {
                            if(isFeedbackButtonNative) {
                                $('body').append('<button class="feedback-btn btn btn-default" title="点我吐槽">' + '<i class="ci-feedback text-primary text-xlg"></i>' + '</button>');
                            }
                            $(document).on('click', settings.feedbackButton, function(){
                               ...
                               /* ctx.fillStyle = 'rgba(102,102,102,0.5)';
                                ...
                                $(document).on('mousedown', '.feedback-sethighlight, .set-highlight', function() {
                                    highlight = 1;
                                    $(this).addClass('feedback-active');
                                    $('.choice-sethighlight').addClass('choice-visible').removeClass('choice-hidden');
                                    $('.feedback-setblackout').removeClass('feedback-active');
                                    $('.choice-setblackout').addClass('choice-hidden').removeClass('choice-visible');
                                    $(this).addClass('active');
                                    $('.set-blackout').removeClass('active');
                                });

                                $(document).on('mousedown', '.feedback-setblackout, .set-blackout', function() {
                                    highlight = 0;
                                    $(this).addClass('feedback-active');
                                    $('.choice-sethighlight').addClass('choice-hidden').removeClass('choice-visible');
                                    $('.feedback-sethighlight').removeClass('feedback-active');
                                    $('.choice-setblackout').addClass('choice-visible').removeClass('choice-hidden');
                                    $(this).addClass('active');
                                    $('.set-highlight').removeClass('active');
                                });
