...
app.service('AreaService', ['$http', '$filter', '$rootScope', '$q',
    function($http, $filter, $rootScope, $q) {
        ...
        var arrMunipalities = [];
        var urlMunipality = 'views/components/conow-area/data/municipalities.json';
        ...
        // 获取热门城市
        var urlHotTopic = 'views/components/conow-area/data/hot-topic.json';
        ...
        var url = 'views/components/conow-area/data/region.json';
        var _this = this;
...
//                arrCityGroup = $filter('areaNameFilter')(arrCityGroup, 'city');
                // 合并以上散步操作，类似链式调用
                arrCityGroup = $filter('areaNameFilter')($filter('orderBy')($filter('areaFilter')(arr, 'city'), 'spell'), 'city');
                ...
                _this.deferred.resolve({ message: "Regions Data is loaded!" });
...
//        area = objData[code][0];
        ...
        // 获取 region.json 文件返回的原始数据:object 类型
