...
function ($compile, $parse, $document, $conowposition, dateFilter, conowdateParser, conowdatepickerPopupConfig, $modal) {
  return {
    ...
    link: function(scope, element, attrs, ngModel) {
      ...
      console.info('this is str' ,eleStr );
...
//        var date = ngModel.$viewValue ? dateFilter(ngModel.$viewValue, dateFormat) : '';
      ...
      element.bind('click', function(e) {
        ...
        $modal.open({
          ...
          template: '<div ng-model="aaa" style="height:1000px">'+eleStr+"</div>"
...
.controller('conowDatepickerController', ['$scope','$filter', function($scope,$filter) {
	...
	var init = function(){
		$scope.isDate = $scope.ngModel instanceof Date;
		if ($scope.isDate) {
			$scope.ngModel = $filter('date')($scope.ngModel, $scope.dateformat);
		}
	...
	var closeWatchFn = $scope.$watch('ngModel', function(newVal, oldVal) {
//		console.info("run to watch ngModel----" + newVal +"---"+ oldVal);
				if (newVal == oldVal) {
					return;
				}
				if(newVal){
...
//		console.info("run to watch conowModel----"+ newVal +"---"+ oldVal);
				if (newVal == oldVal) {
					return;
				}
				closeWatchFn();
...
angular.module("template/conowdatepicker/conowDatepicker.html", []).run(["$templateCache", function($templateCache) {
	  $templateCache.put("template/conowdatepicker/conowDatepicker.html",
		        ...
		        +'<input type="text" class="form-control {{iClass}}" conowdatepicker-popup="{{dateformat}}" ng-blur="ngBlur()" date-mode="{{datemode}}" ng-model="conowModel" is-open="opened" min-date="minDate" max-date="maxDate" datepicker-options="dateOptions" date-disabled="dateDisabled({date:date,mode:mode})" click-fn="clickFn({date:date})" ng-required="ngRequired" ng-click="open($event)" />'
		        +'<span class="input-group-btn">'
		        +'<button type="button" class="btn btn-default {{iClass}}" ng-click="open($event)"><i class="glyphicon glyphicon-calendar"></i></button>'
		        +'</span>'
		        +'</span>'
		        +'<span ng-show="popup">'
		        +'<button type="button" class="btn btn-default" conowdatepicker-popup="{{dateformat}}" ng-blur="ngBlur()" date-mode="{{datemode}}" ng-model="conowModel" is-open="opened" min-date="minDate" max-date="maxDate" datepicker-options="dateOptions" date-disabled="dateDisabled({date:date,mode:mode})" click-fn="clickFn({date:date})" ng-click="open($event)">{{ngModel?ngModel:(value?value:"选择")}}</button>'
		        +'</span>'
		        +'</span>'
	    );
}]);
